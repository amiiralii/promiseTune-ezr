...
def PromiseTune(filename: str, config: PromiseTuneConfig, seed: int) -> Tuple[Optional[Any], List[Any]]:
    ...
    return configuration_b, Rules, result

def run_main(seed: int, name: str) -> None:
    config = PromiseTuneConfig()
    best_configuration, Rules, best_result = PromiseTune(
        filename=f"./Data/{name}",
        config=config,
        seed=seed
    )
    print(f"# Best config: {best_configuration}, Best result: {best_result}")
    # input()
